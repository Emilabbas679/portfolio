    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <div class="col-md-10 offset-md-1">
                <form class="user" method="POST" onsubmit="return validateForm()" name="createService" action="{{ route('service.store') }}" enctype="multipart/form-data">
                    @csrf
...
@push('js')
    ...
    <script>
        function validateForm() {
            var title = document.forms["createService"]["title"].value;
            var icon = document.forms['createService']['icon'].value;
            if(title == ''){
                $('.title-error-js').show('');
            }
            else{
