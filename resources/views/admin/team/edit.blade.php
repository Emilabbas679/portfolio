...
@section('content')
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-sm-flex align-items-center justify-content-between mb-4">
            <h6 class="m-0 font-weight-bold text-primary">Edit {{ $team->name }}</h6>
        </div>
        <div class="card-body">
            <div class="col-md-10 offset-md-1">
                <form class="user" method="POST" action="{{ route('team.update', $team->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        @foreach ($locales as $locale)
                            <div class="tab-pane fade show {{ ($locale->code == 'az') ? 'active': '' }}" id="pills-{{ $locale->code }}"
                                ...
                                <div class="form-group form-row">
                                    ...
                                    <div class="col-md-10">
                                        <input id="position-{{$locale->code}}" type="text" class="form-control @error('position') is-invalid @enderror"
                                               name="position[{{ $locale->code }}]" value="{{ $team->getTranslation('position', $locale->code) }}"
                                               placeholder="{{ __('Position -').$locale->code}}">
                                        @error('position')
                                ...
                                <div class="form-group form-row">
                                    ...
                                    <div class="col-md-10">
                                        <textarea name="content[{{$locale->code}}]" id="" cols="30" rows="10" class='textarea'>
                                            {{$team->getTranslation('content', $locale->code)}}
                                        </textarea>
                                        @error('content')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                    ...
                    <div class="form-group form-row">
                        ...
                        <div class="col-md-10">
                            <input id="name" type="text" class="form-control @error('name') is-invalid @enderror"
                                   name="name" value="{{$team->name}}" placeholder="Name">
