...
@section('content')
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-sm-flex align-items-center justify-content-between mb-4">
            <h6 class="m-0 font-weight-bold text-primary">Add History</h6>
        </div>
        <div class="card-body">
            <div class="col-md-10 offset-md-1">
                <form class="user" method="POST" onsubmit="return validateForm()" name="createHistory" action="{{ route('history.store') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="tab-content pt-2 pl-1" id="pills-tabContent">
                        ...
                        @endforeach
                            <div class="form-group form-row">
                                <div class="col-md-2"><label for="name">Date</label></div>
                                <div class="col-md-10">
                                    <input id="date" type="date" class="form-control @error('date') is-invalid @enderror"
                                           name="date" value="{{ old('date') }}" placeholder="Name">
...
@push('js')
    ...
    <script>
        function validateForm() {
            var img = document.forms["createHistory"]["img"].value;
            var date = document.forms['createHistory']['date'].value;
            if(img == ''){
            ...
            if(date == ''){
                $('.date-error-js').html('Date is required');
                $('.date-error-js').show('');
            }
            else{
                $('.date-error-js').hide('');
